...
key_val_proxy_reg = re.compile(r"^[https]+=[^=]((?:\d{1,3}\.){1,3}\d{1,3}):(\d{1,5})$", re.I)
...
async def langs_support(request):
    ...
    if service:
        ...
        return web.json_response({"langs_service": langs_support, "auth_data": auth_data, "proxies": proxies, "settings":settings})

    return web.json_response({"langs_service": {}, "auth_data":{}, "proxies": proxies, "settings":settings})
...
class DeepTranslatorCLIPTextEncodeNode:
    ...
    @classmethod
    def VALIDATE_INPUTS(cls, from_translate, to_translate, add_proxies, proxies, auth_data, service, text, clip):
...
class DeepTranslatorTextNode(DeepTranslatorCLIPTextEncodeNode):
    ...
    @classmethod
    def VALIDATE_INPUTS(cls, from_translate, to_translate, add_proxies, proxies, auth_data, service, text):
        # lang, is_support, detect = isset_languages(text, service, from_translate, self.langs_support)
        # if not is_support:
        #     return f"[Deep Translator] Service \"{service}\", no support for the provided language: {detect}!"
        return True
